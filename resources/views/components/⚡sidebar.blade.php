...
<div>
    <!-- 1. DESKTOP SIDEBAR (Terkunci & Ditampilkan Hanya di Layar Desktop md:flex) -->
    <aside
        x-data="{
            hoverMode: false,
            isHovered: false,
        ...
        class="hidden md:flex bg-(--bg2-colour) border-r-[1.5px] border-(--outline-colour) flex-col justify-between shrink-0 h-screen transition-all duration-300 ease-in-out select-none relative z-30 overflow-hidden"
    >
        <!-- Bagian Atas: Logo (26px) & Toggle Hover Mode & Menu Navigasi -->
    ...
    <!-- 2. MOBILE SLIDE-OVER DRAWER SIDEBAR (Tampil Saat Hamburger di Top Bar Diklik) -->
    ...
    <div
        ...
        class="fixed inset-0 bg-black/50 z-50 md:hidden backdrop-blur-xs"
        x-cloak
    ></div>
    ...
    <div
        ...
        class="fixed inset-y-0 left-0 w-[280px] bg-(--bg2-colour) z-50 flex flex-col justify-between shadow-2xl md:hidden border-r border-(--outline-colour)"
        x-cloak
    >
        ...
        <div class="flex items-center justify-between px-4 py-4 border-b border-(--outline-colour)/40">
            ...
            <button
                type="button"
                @click="mobileSidebarOpen = false"
                class="p-1.5 rounded-xl text-gray-500 hover:text-gray-800 hover:bg-gray-200/70 transition cursor-pointer"
        ...
        <nav class="space-y-(--size-10) px-3 py-4 flex-1 overflow-y-auto">
            @foreach($navItems as $item)
                @php
                    $isActive = request()->routeIs($item['route'] . '*');
                @endphp
                <div class="w-full rounded-(--size-16) border-[1.5px] transition-all {{ $isActive ? 'text-(--fg-colour) bg-(--prime-colour) border-transparent shadow-xs font-semibold' : 'text-(--text-colour) hover:bg-(--bg-colour) border-transparent hover:border-(--outline-colour)' }}">
                    <a
                        ...
                        @click="mobileSidebarOpen = false"
                        title="{{ $item['label'] }}"
                        class="flex items-center gap-(--size-10) py-(--size-10) px-3.5 font-medium text-sm overflow-hidden whitespace-nowrap transition-all"
                    >
                        <x-dynamic-component :component="$item['icon']" class="w-5 h-5 shrink-0" />
        ...
        <div class="p-3 border-t border-(--outline-colour) bg-(--bg2-colour)/90">
            @auth
                <div class="flex items-center justify-between gap-2.5">
                    <div class="w-9 h-9 rounded-full bg-emerald-100 text-[#163428] flex items-center justify-center font-bold text-sm shrink-0 border border-emerald-300 shadow-xs">
                        <x-lucide-user class="w-4 h-4 text-[#163428]"/>
                    </div>
